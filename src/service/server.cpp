...
void Server::start() {
	...
	app->ws<PerSocketData>(
				 ...
				 {.compression = uWS::SHARED_COMPRESSOR,
					...
					.message =
							[this](etws* ws, std::string_view message, uWS::OpCode opCode) {
								try {
									auto psd =
											reinterpret_cast<PerSocketData*>(ws->getUserData());
									psd->executableName = "";
									auto payload = json::parse(message);
									if (!payload.contains("type") || !payload["type"].is_number())
										return;
									...
									switch (type) {
										case MessageType::HELLO:
											...
											try {
												...
												psd->ready = true;
												LOGDEBUG(
														"WS connected for %s (directory = %s, file = %s)\n",
...
void Server::update_style(std::string& exeName, std::string& styleContent) {
	LOGDEBUG("Updating style for %s - styles %zi length\n", exeName.c_str(),
					 styleContent.size());
	json p = {{"type", MessageType::STYLES_UPDATE}, {"css", styleContent}};
